...
```{r performance-predictors, message=FALSE, warning=FALSE}
library(glmmTMB)
library(broom.mixed)
library(ggplot2)
library(dplyr)
...
# Function to model and plot per condition
analyze_condition <- function(df_cond, cond_label) {
  # Beta transformation of response
  n <- nrow(df_cond)
  df_cond[[response]] <- (df_cond[[response]] * (n - 1) + 0.5) / n
  
  for (pred in predictors) {
    form <- as.formula(paste(response, "~", pred, "+ VBMAPP + time_c + (1 | Subject)"))
    model <- glmmTMB(form, data = df_cond, family = beta_family())
    results <- broom.mixed::tidy(model, effects = "fixed")
    results$stars <- sapply(results$p.value, sig_stars)

    cat("\n====================\n")
    cat("Condition:", cond_label, "| Predictor:", pred, "\n")
    print(results[, c("term", "estimate", "std.error", "statistic", "p.value", "stars")])
    
    # Generate predictions for plotting
    new_data <- expand.grid(
      pred_val = seq(min(df_cond[[pred]], na.rm = TRUE), max(df_cond[[pred]], na.rm = TRUE), length.out = 100),
      VBMAPP = mean(df_cond$VBMAPP, na.rm = TRUE),
      time_c = 0,
      Subject = NA
    )
    ...
    # Create plot
    p <- ggplot(new_data, aes_string(x = pred, y = "predicted")) +
    ...
    # Save to PDF
    pdf_file <- paste0("figs/predictor_", pred, "_", cond_label, ".pdf")
    ggsave(pdf_file, plot = p, width = 6, height = 4)
...
# Run analysis separately for Human and Robot
analyze_condition(df %>% filter(Condition == "Human"), "Human")
analyze_condition(df %>% filter(Condition == "Robot"), "Robot")
```
...
# Convert condition to factor and center time
df$Condition <- factor(df$Condition, levels = c(1, 2), labels = c("Human", "Robot"))
df$time_c <- scale(df$time, center = TRUE, scale = FALSE)
...
# Beta transform
n <- nrow(df)
df[[response]] <- (df[[response]] * (n - 1) + 0.5) / n

# Loop through predictors
for (pred in predictors) {
  form <- as.formula(paste(response, "~", pred, "* Condition + VBMAPP + time_c + (1 | Subject)"))
  model <- glmmTMB(form, data = df, family = beta_family())
  results <- broom.mixed::tidy(model, effects = "fixed")
  results$stars <- sapply(results$p.value, sig_stars)
  ...
  # Prediction data
  new_data <- expand.grid(
    pred_val = seq(min(df[[pred]], na.rm = TRUE), max(df[[pred]], na.rm = TRUE), length.out = 100),
    Condition = factor(c("Human", "Robot"), levels = c("Human", "Robot")),
    VBMAPP = mean(df$VBMAPP, na.rm = TRUE),
    time_c = 0,
    Subject = NA
  ...
  # Combined plot
  p <- ggplot(new_data, aes_string(x = pred, y = "predicted", color = "Condition", fill = "Condition")) +
    geom_line(size = 1) +
  ...
  pdf_file <- paste0("figs/predictor_", pred, "_combined.pdf")
  ggsave(pdf_file, plot = p, width = 6, height = 4)
