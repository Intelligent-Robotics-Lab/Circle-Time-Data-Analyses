...
```{r radar-plot-affect-positive, message=FALSE, warning=FALSE, fig.width=6, fig.height=6}
# Install and load fmsb
...
# Ensure Condition is labeled properly
df$Condition <- factor(df$Condition, levels = c(1, 2), labels = c("Human", "Robot"))
df$time_c <- scale(df$time, center = TRUE, scale = FALSE)

# Compute mean Affect_Positive per participant per condition
avg_affect <- df %>%
  group_by(Subject, Condition) %>%
  summarise(Affect_Positive = mean(Affect_Positive, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = Condition, values_from = Affect_Positive)

# Sort by Subject ID
avg_affect <- avg_affect[order(avg_affect$Subject), ]

# Extract values
human <- avg_affect$Human
robot <- avg_affect$Robot
subjects <- avg_affect$Subject

# Create data frame for radar chart
data <- as.data.frame(rbind(
  rep(1, length(human)),  # max
  rep(0, length(human)),  # min
  human,
  robot
))
colnames(data) <- subjects
...
radarchart(data,
  ...
  axislabcol = "grey",
  vlcex = 0.8,
  title = "Affect_Positive by Participant: Human (blue) vs. Robot (red)"
)
```
...
```{r radar-plot-affect-negative, message=FALSE, warning=FALSE, fig.width=6, fig.height=6}
# Install and load fmsb
if (!requireNamespace("fmsb", quietly = TRUE)) {
  install.packages("fmsb")
}
...
avg_affect <- df %>%
  group_by(Subject, Condition) %>%
  summarise(Affect_Negative = mean(Affect_Negative, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = Condition, values_from = Affect_Negative)
...
radarchart(data,
  ...
  axislabcol = "grey",
  vlcex = 0.8,
  title = "Affect_Negative by Participant: Human (blue) vs. Robot (red)"
)
``` 
...
```{r radar-plot-comm-instructor, message=FALSE, warning=FALSE, fig.width=6, fig.height=6}
# Install and load fmsb
if (!requireNamespace("fmsb", quietly = TRUE)) {
  install.packages("fmsb")
}
...
# Select variable
comm_var <- "Communication_with_Instructor"

# Compute mean per participant/condition
avg_comm <- df %>%
  group_by(Subject, Condition) %>%
  summarise(value = mean(.data[[comm_var]], na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = Condition, values_from = value)

avg_comm <- avg_comm[order(avg_comm$Subject), ]

# Prepare values
human <- avg_comm$Human
robot <- avg_comm$Robot
subjects <- avg_comm$Subject
...
oldpar <- par(no.readonly = TRUE)
par(cex.main = 0.9)  # smaller title font
...
radarchart(data,
  ...
  axislabcol = "grey",
  vlcex = 0.8,
  title = "Communication with Instructor by Participant: Human (blue) vs. Robot (red)"
)
